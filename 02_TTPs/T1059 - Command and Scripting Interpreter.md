...
| S9032 | MuddyViper | MuddyViper has launched a reverse shell using a provided command line. [36] |
| G0129 | Mustang Panda | Mustang Panda has utilized meterpreter shellcode. [37] |
| S1192 | NICECURL | NICECURL has provided an arbitrary command execution interface. [38] |
| G0049 | OilRig | OilRig has used various types of scripting for execution. [39] [40] [41] [42] [43] |
| C0005 | Operation Spalax | For Operation Spalax , the threat actors used Nullsoft Scriptable Install System (NSIS) scripts to install malware. [44] |
| S0598 | P.A.S. Webshell | P.A.S. Webshell has the ability to create reverse shells with Perl scripts. [45] |
| S1130 | Raspberry Robin | Raspberry Robin variants can be delivered via highly obfuscated Windows Script Files (WSF) for initial execution. [46] |
| G1031 | Saint Bear | Saint Bear has used the Windows Script Host (wscript) to execute intermediate files written to victim machines. [47] |
| S1110 | SLIGHTPULSE | SLIGHTPULSE contains functionality to execute arbitrary commands passed to it. [48] |
| S0374 | SpeakUp | SpeakUp uses Perl scripts. [49] |
| S1227 | StarProxy | StarProxy has used the command line for execution of commands. [50] |
| G0038 | Stealth Falcon | Stealth Falcon malware uses WMI to script data collection and command execution on the victim. [51] |
| S1154 | VersaMem | VersaMem was delivered as a Java Archive (JAR) that runs by attaching itself to the Apache Tomcat Java servlet and web server. [52] |
| G0107 | Whitefly | Whitefly has used a simple remote shell tool that will call back to the C2 server and wait for commands. [53] |
| G0124 | Windigo | Windigo has used a Perl script for information gathering. [11] |
| S0219 | WINERACK | WINERACK can create a reverse shell that utilizes statically-linked Wine cmd.exe code to emulate Windows command prompt commands. [54] |
| G1035 | Winter Vivern | Winter Vivern used XLM 4.0 macros for initial code execution for malicious document files. [55] |
| S1151 | ZeroCleare | ZeroCleare can receive command line arguments from an operator to corrupt the file system using the RawDisk driver. [56] |
| S0330 | Zeus Panda | Zeus Panda can launch remote scripts on the victim’s machine. [57] |
...
| M1040 | Behavior Prevention on Endpoint | On Windows 10, enable Attack Surface Reduction (ASR) rules to prevent Visual Basic and JavaScript scripts from executing potentially malicious downloaded content [58] . |
...
| M1038 | Execution Prevention | Use application control where appropriate. For example, PowerShell Constrained Language mode can be used to restrict access to sensitive or otherwise dangerous language elements such as those used to execute arbitrary Windows APIs or files (e.g., Add-Type ). [59] |
| M1033 | Limit Software Installation | Prevent user installation of unrequired command and scripting interpreters. |
| M1026 | Privileged Account Management | When PowerShell is necessary, consider restricting PowerShell execution policy to administrators. Be aware that there are methods of bypassing the PowerShell execution policy, depending on environment configuration. [60] PowerShell JEA (Just Enough Administration) may also be used to sandbox administration and limit what commands admins/users can execute through remote PowerShell sessions. [61] |
